...
func (h *Host) manangedDownloadIteration(so *storageObligation) error {
	...
	err = modules.ReadNegotiateAcceptance(conn)
	if err != nil {
		return err
	}

	// Read a batch of download requests.
	var requests []modules.DownloadRequest
...
func (h *Host) managedRPCDownload(conn net.Conn) error {
	...
	for time.Now().Before(startTime.Add(1200 * time.Second)) {
		err := h.managedDownloadIteration(so)
