...
type (
	...
	savedSettings struct {
		SectorLocations []sectorLocationEntry
...
func (cm *ContractManager) initSettings() error {
	...
	return persist.SaveFileSync(settingsMetadata, cm.saveData(), filepath.Join(cm.persistDir, settingsFile))
}
...
func (cm *ContractManager) load() error {
	...
	for i := range ss.SectorLocations {
		cm.sectorLocations[ss.SectorLocations[i].sectorID] = sectorLocation{
			index:         ss.SectorLocations[i].index,
			storageFolder: ss.SectorLocations[i].storageFolder,
		}
	}
...
// saveData returns a savedSettings struct that is ready to be committed to
// disk.
func (cm *ContractManager) saveData() savedSettings {
	ss := savedSettings{
	...
	var i int
	ss.SectorLocations = make([]sectorLocationEntry, len(cm.sectorLocations))
	for id, location := range cm.sectorLocations {
		ss.SectorLocations[i] = sectorLocationEntry{
			index:         location.index,
			sectorID:      id,
			storageFolder: location.storageFolder,
		}
		i++
	}
	return ss
