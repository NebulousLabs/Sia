...
// COMPATv1.3.0
// v130managedRevisionIteration handles one iteration of the revision loop. As a
// performance optimization, multiple iterations of revisions are allowed to be
// made over the same connection.
func (h *Host) v130managedRevisionIteration(conn net.Conn, so *storageObligation, finalIter bool) error {
	...
	var modifications []modules.V130RevisionAction
	...
	err = func() error {
		...
		return extendErr("unable to verify updated contract: ", verifyRevision(*so, revision, blockHeight, newRevenue, newCollateral, true))
	}()
	if err != nil {
...
// COMPATv1.3.0
// v130managedRPCReviseContract accepts a request to revise an existing contract.
// Revisions can add sectors, delete sectors, and modify existing sectors.
func (h *Host) v130managedRPCReviseContract(conn net.Conn) error {
	...
	for timeoutReached := false; !timeoutReached; {
		...
		err := h.v130managedRevisionIteration(conn, &so, timeoutReached)
		if err == modules.ErrStopResponse {
			return nil
		} else if err != nil {
