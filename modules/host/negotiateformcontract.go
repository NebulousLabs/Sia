...
func (h *Host) managedRPCFormContract(conn net.Conn) error {
	...
	h.mu.RUnlock()
	if !eSettings.AcceptingContracts {
	...
	txnBuilder, newParents, newInputs, newOutputs, err := h.managedAddCollateral(eSettings, txnSet)
	if err != nil {
		modules.WriteNegotiationRejection(conn, err) // Error ignored to preserve type in extendErr
	...
	h.mu.RLock()
	hostCollateral := contractCollateral(eSettings, txnSet[len(txnSet)-1].FileContracts[0])
	h.mu.RUnlock()
