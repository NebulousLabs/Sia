...
// setChildTarget computes the target of a headerNode's child. All children of a node
// have the same target.
...
func (cs *ConsensusSet) newHeader(tx *bolt.Tx, parentHeader *processedHeader, header types.BlockHeader) *processedHeader {
	...
	if parentHeader.Height < types.OakHardforkBlock {
		cs.setHeaderChildTarget(headerMap, child)
	} else {
		child.ChildTarget = cs.childTargetOak(prevTotalTime, prevTotalTarget, parentHeader.ChildTarget, parentHeader.Height)
	}
