...
// reviseContract revises the previous revision to incorporate new data and
// uploads both the revision and the piece data to the host.
func (hu *hostUploader) reviseContract(rev types.FileContractRevision, piece []byte, height types.BlockHeight) error {
	...
	piecePrice := types.NewCurrency64(uint64(len(piece))).Mul(types.NewCurrency64(safeDuration)).Mul(hu.price)
