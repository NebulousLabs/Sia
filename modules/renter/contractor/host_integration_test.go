...
func TestRenewFailing(t *testing.T) {
	...
	allowance := modules.Allowance{
		RenewWindow: 2 * consecutiveRenewalsBeforeReplacement,
		Hosts:       1,
		Funds:       types.SiacoinPrecision.Mul64(100000),
		Period:      50,
	}
	if err := c.SetAllowance(allowance); err != nil {
		t.Fatal(err)
	}

	// wait for a contract to be formed
	err = build.Retry(100, 100, func() error {
		...
		return errors.New("no contract was formed")
	})
	if err != nil {
		t.Fatal(err)
	}
	...
	c.mu.Unlock()
	for blockHeight+renewWindow < contract.EndHeight {
		if _, err := m.AddBlock(); err != nil {
			t.Fatal(err)
		}
		blockHeight++
	...
	err = build.Retry(600, 100*time.Millisecond, func() error {
		...
		cu, ok = c.ContractUtility(contract.ID)
		if !ok {
			return errors.New("failed to get utility")
		}
		if cu.GoodForRenew {
