...
Transaction signatures include more than just a cryptographic signature.
A time lock is also specified (performing the same function as in a spend condition), and an input ID must be provided as well.
This ID must match one of the inputs in the transaction.
Finally, a bit array is used to indicate which fields in the transaction are being signed.
Nearly any field in the transaction can be referenced by this array: the version number, the arbitrary data, the file contract or storage proof, or any combination of inputs and outputs.
...
The contract is created by splitting the file into segments and calculating their corresponding Merkle root hash.
...
The first such window, $W_0$, begins $N$ blocks after the block containing the contract.

The storage challenge requires a random number as a seed.
The random number for window $W_i$ is generated by concatenating contract ID with the block prior to $W_i$ (i.e. $W_0 + i*N$) and hashing the result.
...
The random number generator is subject to manipulation via block withholding attakcs.
